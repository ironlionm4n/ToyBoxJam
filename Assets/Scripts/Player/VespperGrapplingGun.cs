...
public class VespperGrapplingGun : MonoBehaviour
{
    [Header("Scripts Ref:")] public VesperGrapplingRope grappleRope;
    ...
    [SerializeField] private float ropeDelta;
        ...
        else if (Input.GetKey(KeyCode.Mouse1))
        {
            if (grappleRope.enabled)
            {
                if (Input.GetKey(KeyCode.W))
                    m_springJoint2D.distance -= ropeDelta * Time.deltaTime;
                else if (Input.GetKey(KeyCode.S)) m_springJoint2D.distance += ropeDelta * Time.deltaTime;
            ...
            if (launchToPoint && grappleRope.isGrappling)
                if (launchType == LaunchType.Transform_Launch)
                {
                    ...
                    var targetPos = grapplePoint - firePointDistnace;
    ...
    private void SetGrapplePoint()
    {
        ...
        Vector2 distanceVector = mousePos - gunPivot.position;
        var circleHit = Physics2D.OverlapCircle(mousePos, circleCastRadius, grappleLayer);

        var clearanceHit = Physics2D.Raycast(gunHolder.position, distanceVector.normalized);

        if (clearanceHit)
            if (clearanceHit.transform.gameObject != null)
            {
                if (clearanceHit.transform.gameObject.layer != grappableLayerNumber) return;
                if (circleHit)
                    if (Vector2.Distance(circleHit.transform.position, firePoint.position) <= maxDistnace ||
                        !hasMaxDistance)
                    {
                        grapplePoint = circleHit.transform.position;
        ...
        else
        {
            switch (launchType)
            {
                case LaunchType.Physics_Launch:
                    ...
                    m_springJoint2D.distance = distanceVector.magnitude * 2f;
